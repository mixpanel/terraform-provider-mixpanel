...
```
                inputs                                        generated output
   ┌───────────────────────────────────┐
   │ gen/spec/openapi.pruned.json       │  (LOCAL ONLY, gitignored, 504 paths —
   │                                    │   fetch + prune before regen; never committed)
   │ gen/refined_manifest.json          │  (roster/knob, committed)
   └───────────────┬───────────────────┘
                   │
...
| `provider_code_spec.json` | The HashiCorp IR (typed schemas) — committed so a stage-4 regen needs no binaries | ✅ committed |
...
| `spec/*.json` | OpenAPI inputs — the full `openapi.merged.json` and the pruned `openapi.pruned.json`. **Deliberately NOT committed** (they map the API surface); fetch + prune them locally before a regen. | ❌ gitignored |
...
Requires `python3` + `gofmt` (**no external binaries**) **plus a local
`gen/spec/openapi.pruned.json`** — the spec is gitignored and not shipped, so
fetch + prune it first (see [Refreshing the spec](#refreshing-the-frozen-spec)).
`crudgen.py` reads only its `components/schemas` (for wire-key casing), never its
paths. Touches `internal/provider/*.go` and `examples/`.
...
The spec is intentionally **decoupled** from the live API and **not committed**
to this repo (it maps the API surface). Both `gen/spec/*.json` files are
gitignored, so producing them locally is a required first step before any regen:

1. Obtain the full merged OpenAPI spec from the webapp
   (`webapp/ninja/generated/openapi.merged.json`).
2. Drop it in as `gen/spec/openapi.merged.json` (gitignored).
3. Re-run the prune:
