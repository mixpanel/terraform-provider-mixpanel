...
import (
	...
	rsc "github.com/mixpanel/terraform-provider-mixpanel/internal/provider/resource_annotation"
)
...
// NewAnnotationResource constructs the annotation resource.
...
// projectID resolves the project from the annotation project_id attribute (if any)
// falling back to the provider default.
func (r *AnnotationResource) projectID(ctx context.Context, raw tftypes.Value) (string, error) {
...
func (r *AnnotationResource) collectionPath(projectID string) string {
	return strings.NewReplacer("{project_id}", projectID).Replace("/api/app/projects/{project_id}/annotations")
}

func (r *AnnotationResource) instancePath(projectID, id string) string {
	return strings.NewReplacer("{project_id}", projectID, "{annotation_id}", id).Replace("/api/app/projects/{project_id}/annotations/{annotation_id}")
}

func (r *AnnotationResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	spec := AnnotationAttrSpec()
	...
	if err != nil {
		resp.Diagnostics.AddError("Encoding annotation request", err.Error())
		return
	}
	respBody, err := r.client.Do(ctx, "POST", r.collectionPath(projectID), body)
	if err != nil {
		resp.Diagnostics.AddError("Creating annotation", err.Error())
		return
	}
	wire, err := unwrapAnnotation(respBody)
	if err != nil {
...
func (r *AnnotationResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Reading annotation", err.Error())
		return
	}
	wire, err := unwrapAnnotation(respBody)
	if err != nil {
...
func (r *AnnotationResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError("Updating annotation", err.Error())
		return
	}
	wire, err := unwrapAnnotation(respBody)
	if err != nil {
...
func (r *AnnotationResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if _, err := r.client.Do(ctx, "DELETE", r.instancePath(projectID, id), nil); err != nil {
		...
		resp.Diagnostics.AddError("Deleting annotation", err.Error())
		return
	}
}
...
// idForAnnotation extracts the identity value from an unwrapped response body.
func idForAnnotation(wire map[string]any) string {
...
// unwrapAnnotation unwraps the API envelope (when enveloped) and returns the body map.
func unwrapAnnotation(respBody []byte) (map[string]any, error) {
