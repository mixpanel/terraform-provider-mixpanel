...
import (
	...
	rsc "github.com/mixpanel/terraform-provider-mixpanel/internal/provider/resource_custom_event"
)

var (
	_ resource.Resource                = (*CustomEventResource)(nil)
	_ resource.ResourceWithConfigure   = (*CustomEventResource)(nil)
	_ resource.ResourceWithImportState = (*CustomEventResource)(nil)
)
...
// NewCustomEventResource constructs the custom_event resource.
...
// projectID resolves the project from the custom_event project_id attribute (if any)
// falling back to the provider default.
func (r *CustomEventResource) projectID(ctx context.Context, raw tftypes.Value) (string, error) {
...
func (r *CustomEventResource) collectionPath(projectID string) string {
	return strings.NewReplacer("{project_id}", projectID).Replace("/api/app/custom_events/{project_id}/")
}

func (r *CustomEventResource) instancePath(projectID, id string) string {
	return strings.NewReplacer("{project_id}", projectID, "{customevent_id}", id).Replace("/api/app/custom_events/{project_id}/{customevent_id}/")
}
...
func (r *CustomEventResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	respBody, err := r.client.DoForm(ctx, "POST", r.collectionPath(projectID), body)
	if err != nil {
		resp.Diagnostics.AddError("Creating custom_event", err.Error())
		return
	}
	wire, err := unwrapCustomEvent(respBody)
	if err != nil {
...
func (r *CustomEventResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	id, err := stringAttrFromRaw(req.State.Raw, "customevent_id")
	if err != nil {
...
func (r *CustomEventResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	respBody, err := r.client.DoForm(ctx, "PUT", r.instancePath(projectID, id), body)
	if err != nil {
		resp.Diagnostics.AddError("Updating custom_event", err.Error())
		return
	}
	wire, err := unwrapCustomEvent(respBody)
	if err != nil {
...
func (r *CustomEventResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if _, err := r.client.Do(ctx, "DELETE", r.instancePath(projectID, id), nil); err != nil {
		...
		resp.Diagnostics.AddError("Deleting custom_event", err.Error())
		return
	}
}
...
// idForCustomEvent extracts the identity value from an unwrapped response body.
func idForCustomEvent(wire map[string]any) string {
	if v, ok := client.IDFromWire(wire, "customevent_id"); ok {
		return v
	}
	if v, ok := nestedID(wire, "custom_event.id"); ok {
		return v
	}
	return ""
}

// unwrapCustomEvent unwraps the API envelope (when enveloped) and returns the body map.
func unwrapCustomEvent(respBody []byte) (map[string]any, error) {
	body, err := unwrapBody(respBody, false)
	if err != nil {
		return nil, err
	}
