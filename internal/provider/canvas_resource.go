...
import (
	...
	rsc "github.com/mixpanel/terraform-provider-mixpanel/internal/provider/resource_canvas"
)
...
// NewCanvasResource constructs the canvas resource.
...
func (r *CanvasResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	s := rsc.CanvasResourceSchema(ctx)
...
// projectID resolves the project from the canvas project_id attribute (if any)
// falling back to the provider default.
func (r *CanvasResource) projectID(ctx context.Context, raw tftypes.Value) (string, error) {
...
func (r *CanvasResource) collectionPath(projectID string) string {
	return strings.NewReplacer("{project_id}", projectID).Replace("/api/app/projects/{project_id}/canvases")
}

func (r *CanvasResource) instancePath(projectID, id string) string {
	return strings.NewReplacer("{project_id}", projectID, "{canvas_id}", id).Replace("/api/app/projects/{project_id}/canvases/{canvas_id}")
}
...
func (r *CanvasResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError("Creating canvas", err.Error())
		return
	}
	wire, err := unwrapCanvas(respBody)
	if err != nil {
		resp.Diagnostics.AddError("Decoding canvas response", err.Error())
		return
	}
	id := idForCanvas(wire)
	r.writeCanvasState(ctx, &resp.State, &resp.Diagnostics, client.MergeApply, req.Plan.Raw, wire, projectID, id)
}
...
func (r *CanvasResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	r.writeCanvasState(ctx, &resp.State, &resp.Diagnostics, client.MergeRead, req.State.Raw, wire, projectID, id)
}
...
func (r *CanvasResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError("Updating canvas", err.Error())
		return
	}
	wire, err := unwrapCanvas(respBody)
	if err != nil {
...
func (r *CanvasResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if _, err := r.client.Do(ctx, "DELETE", r.instancePath(projectID, id), nil); err != nil {
		...
		resp.Diagnostics.AddError("Deleting canvas", err.Error())
		return
	}
}
...
// writeCanvasState turns an unwrapped API body into resource state. On
...
func (r *CanvasResource) writeCanvasState(ctx context.Context, state *tfsdk.State, diags *diagAppender, mode client.MergeMode, base tftypes.Value, wire map[string]any, projectID, id string) {
	...
	val, err := client.RawFromWireMerged(schemaType, mode, base, wire, extras, CanvasAttrSpec())
	if err != nil {
		diags.AddError("Building canvas state", err.Error())
		return
	}
	state.Raw = val
}

// idForCanvas extracts the identity value from an unwrapped response body.
func idForCanvas(wire map[string]any) string {
...
// unwrapCanvas unwraps the API envelope (when enveloped) and returns the body map.
func unwrapCanvas(respBody []byte) (map[string]any, error) {
