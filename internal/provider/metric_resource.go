...
func (r *MetricResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	id := idForMetric(wire)
	r.writeMetricState(ctx, &resp.State, &resp.Diagnostics, req.Plan.Raw, wire, projectID, id)
...
func (r *MetricResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	r.writeMetricState(ctx, &resp.State, &resp.Diagnostics, req.State.Raw, wire, projectID, id)
...
// writeMetricState turns an unwrapped API body into resource state. base is the
...
func (r *MetricResource) writeMetricState(ctx context.Context, state *tfsdk.State, diags *diagAppender, base tftypes.Value, wire map[string]any, projectID, id string) {
	...
	val, err := client.RawFromWireMerged(schemaType, base, wire, extras, MetricAttrSpec())
	if err != nil {
