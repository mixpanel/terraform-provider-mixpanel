...
import (
	...
	rsc "github.com/mixpanel/terraform-provider-mixpanel/internal/provider/resource_metric"
)
...
// NewMetricResource constructs the metric resource.
...
func (r *MetricResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	s := rsc.MetricResourceSchema(ctx)
...
// projectID resolves the project from the metric project_id attribute (if any)
// falling back to the provider default.
func (r *MetricResource) projectID(ctx context.Context, raw tftypes.Value) (string, error) {
...
func (r *MetricResource) collectionPath(projectID string) string {
...
func (r *MetricResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError("Encoding metric request", err.Error())
		return
	}
	respBody, err := r.client.Do(ctx, "POST", r.collectionPath(projectID), body)
	if err != nil {
		resp.Diagnostics.AddError("Creating metric", err.Error())
		return
	}
	wire, err := unwrapMetric(respBody)
	if err != nil {
		resp.Diagnostics.AddError("Decoding metric response", err.Error())
		return
	}
	id := idForMetric(wire)
	r.writeMetricState(ctx, &resp.State, &resp.Diagnostics, client.MergeApply, req.Plan.Raw, wire, projectID, id)
...
func (r *MetricResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	r.writeMetricState(ctx, &resp.State, &resp.Diagnostics, client.MergeRead, req.State.Raw, wire, projectID, id)
...
func (r *MetricResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError("Updating metric", err.Error())
		return
	}
	wire, err := unwrapMetric(respBody)
	if err != nil {
...
func (r *MetricResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if _, err := r.client.Do(ctx, "DELETE", r.instancePath(projectID, id), nil); err != nil {
		...
		resp.Diagnostics.AddError("Deleting metric", err.Error())
		return
	}
}
...
// writeMetricState turns an unwrapped API body into resource state. On
...
func (r *MetricResource) writeMetricState(ctx context.Context, state *tfsdk.State, diags *diagAppender, mode client.MergeMode, base tftypes.Value, wire map[string]any, projectID, id string) {
	...
	val, err := client.RawFromWireMerged(schemaType, mode, base, wire, extras, MetricAttrSpec())
	if err != nil {
		diags.AddError("Building metric state", err.Error())
		return
	}
	state.Raw = val
}

// idForMetric extracts the identity value from an unwrapped response body.
func idForMetric(wire map[string]any) string {
...
// unwrapMetric unwraps the API envelope (when enveloped) and returns the body map.
func unwrapMetric(respBody []byte) (map[string]any, error) {
