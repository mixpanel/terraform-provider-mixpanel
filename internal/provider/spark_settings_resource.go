...
import (
	...
	rsc "github.com/mixpanel/terraform-provider-mixpanel/internal/provider/resource_spark_settings"
)

var (
	_ resource.Resource                = (*SparkSettingsResource)(nil)
	_ resource.ResourceWithConfigure   = (*SparkSettingsResource)(nil)
	_ resource.ResourceWithImportState = (*SparkSettingsResource)(nil)
)
...
// NewSparkSettingsResource constructs the spark_settings resource.
func NewSparkSettingsResource() resource.Resource {
	return &SparkSettingsResource{}
}

type SparkSettingsResource struct {
	client *client.Client
}

func (r *SparkSettingsResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
...
func (r *SparkSettingsResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
// projectID resolves the project from the spark_settings project_id attribute (if any)
// falling back to the provider default.
func (r *SparkSettingsResource) projectID(ctx context.Context, raw tftypes.Value) (string, error) {
...
func (r *SparkSettingsResource) collectionPath(projectID string) string {
	return strings.NewReplacer("{organization_id}", projectID).Replace("/organizations/{organization_id}/spark-settings")
}

func (r *SparkSettingsResource) instancePath(projectID, id string) string {
	return strings.NewReplacer("{organization_id}", projectID, "{id}", id).Replace("/organizations/{organization_id}/spark-settings")
}

func (r *SparkSettingsResource) updatePath(scopeID string) string {
	return strings.NewReplacer("{organization_id}", scopeID).Replace("/organizations/{organization_id}/update-spark-settings")
}

func (r *SparkSettingsResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError("Reading spark_settings after create", err.Error())
		return
	}
	wire, err := unwrapSparkSettings(getBody)
	if err != nil {
...
func (r *SparkSettingsResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	wire, err := unwrapSparkSettings(respBody)
	if err != nil {
		resp.Diagnostics.AddError("Decoding spark_settings response", err.Error())
		return
	}
...
func (r *SparkSettingsResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	getBody, err := r.client.Do(ctx, "GET", r.collectionPath(scopeID), nil)
	if err != nil {
		resp.Diagnostics.AddError("Reading spark_settings after update", err.Error())
		return
	}
	wire, err := unwrapSparkSettings(getBody)
	if err != nil {
...
func (r *SparkSettingsResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	// spark_settings is a project singleton: destroy removes it from state only and does
...
func (r *SparkSettingsResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	if req.ID == "" {
		resp.Diagnostics.AddError(
			"Invalid import ID",
...
// idForSparkSettings extracts the identity value from an unwrapped response body.
func idForSparkSettings(wire map[string]any) string {
...
// unwrapSparkSettings unwraps the API envelope (when enveloped) and returns the body map.
func unwrapSparkSettings(respBody []byte) (map[string]any, error) {
