// Code generated by gen/crudgen.py. DO NOT EDIT.
//
// HAND-EDITED EXCEPTION: "share_with_project" added to PathParamAttrs so the
// synthetic sharing attribute (see sharing.go) is never serialized into
// entity request bodies. Re-apply if regenerating.

package provider

import "github.com/mixpanel/terraform-provider-mixpanel/internal/client"

// CustomPropertyAttrSpec describes the synthetic / jsonencode attributes of the custom_property
// entity to the generic tftypes<->JSON bridge in the client package. It is shared
// by the custom_property resource and data source.
func CustomPropertyAttrSpec() client.AttrSpec {
	return client.AttrSpec{
		IDAttr:              "custom_property_id",
		ProjectIDAttr:       "project_id",
		PathParamAttrs:      map[string]bool{"share_with_project": true},
		JSONEncodeAttrs:     map[string]bool{"behavior": true, "composed_properties": true, "display_options": true},
		JSONStringAttrs:     map[string]bool{},
		JSONEncodeWireKey:   map[string]string{"can_update_basic": "canUpdateBasic", "composed_properties": "composedProperties", "data_group_id": "dataGroupId", "display_formula": "displayFormula", "display_options": "displayOptions", "example_value": "exampleValue", "global_access_type": "globalAccessType", "is_locked": "isLocked", "is_session_scoped": "isSessionScoped", "is_visible": "isVisible", "join_property_type": "joinPropertyType", "mapped_data_group_id": "mappedDataGroupId", "property_type": "propertyType", "referenced_by": "referencedBy", "referenced_directly_by": "referencedDirectlyBy", "referenced_raw_event_properties": "referencedRawEventProperties", "references_borrowed_property": "referencesBorrowedProperty", "resource_type": "resourceType"},
		OutputOnlyAttrs:     map[string]bool{"can_update_basic": true, "created": true, "is_session_scoped": true, "join_property_type": true, "mapped_data_group_id": true, "modified": true, "project": true, "property_type": true, "referenced_by": true, "referenced_directly_by": true, "referenced_raw_event_properties": true, "references_borrowed_property": true, "user": true},
		SpreadAttrs:         map[string]bool{},
		CreateWritableAttrs: map[string]bool{"behavior": true, "composed_properties": true, "data_group_id": true, "description": true, "display_formula": true, "display_options": true, "example_value": true, "global_access_type": true, "is_locked": true, "is_visible": true, "name": true, "resource_type": true},
		UpdateWritableAttrs: map[string]bool{"behavior": true, "composed_properties": true, "data_group_id": true, "description": true, "display_formula": true, "display_options": true, "example_value": true, "is_locked": true, "is_visible": true, "name": true, "resource_type": true},
	}
}
