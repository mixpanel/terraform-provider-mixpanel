...
import (
	...
	rsc "github.com/mixpanel/terraform-provider-mixpanel/internal/provider/resource_webhook"
)
...
// NewWebhookResource constructs the webhook resource.
...
func (r *WebhookResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	s := rsc.WebhookResourceSchema(ctx)
...
// projectID resolves the project from the webhook project_id attribute (if any)
// falling back to the provider default.
func (r *WebhookResource) projectID(ctx context.Context, raw tftypes.Value) (string, error) {
...
func (r *WebhookResource) collectionPath(projectID string) string {
	return strings.NewReplacer("{project_id}", projectID).Replace("/api/app/projects/{project_id}/webhooks")
}

func (r *WebhookResource) instancePath(projectID, id string) string {
	return strings.NewReplacer("{project_id}", projectID, "{webhook_id}", id).Replace("/api/app/projects/{project_id}/webhooks/{webhook_id}")
}

func (r *WebhookResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	spec := WebhookAttrSpec()
	...
	if err != nil {
		resp.Diagnostics.AddError("Encoding webhook request", err.Error())
		return
	}
	respBody, err := r.client.Do(ctx, "POST", r.collectionPath(projectID), body)
	if err != nil {
		resp.Diagnostics.AddError("Creating webhook", err.Error())
		return
	}
	wire, err := unwrapWebhook(respBody)
	if err != nil {
...
func (r *WebhookResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError("Reading webhook id", err.Error())
		return
	}
...
func (r *WebhookResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if _, err := r.client.Do(ctx, "DELETE", r.instancePath(projectID, id), nil); err != nil {
		...
		resp.Diagnostics.AddError("Deleting webhook", err.Error())
		return
	}
}
...
// idForWebhook extracts the identity value from an unwrapped response body.
func idForWebhook(wire map[string]any) string {
...
// unwrapWebhook unwraps the API envelope (when enveloped) and returns the body map.
func unwrapWebhook(respBody []byte) (map[string]any, error) {
