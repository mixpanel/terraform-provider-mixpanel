// Code generated by gen/crudgen.py. DO NOT EDIT.
//
// HAND-EDITED EXCEPTION: "share_with_project" added to PathParamAttrs so the
// synthetic sharing attribute (see sharing.go) is never serialized into
// entity request bodies. Re-apply if regenerating.

package provider

import "github.com/mixpanel/terraform-provider-mixpanel/internal/client"

// FormulaAttrSpec describes the synthetic / jsonencode attributes of the formula
// entity to the generic tftypes<->JSON bridge in the client package. It is shared
// by the formula resource and data source.
func FormulaAttrSpec() client.AttrSpec {
	return client.AttrSpec{
		IDAttr:              "id",
		ProjectIDAttr:       "project_id",
		PathParamAttrs:      map[string]bool{"share_with_project": true, "metric_id": true},
		JSONEncodeAttrs:     map[string]bool{"definition": true},
		JSONStringAttrs:     map[string]bool{},
		JSONEncodeWireKey:   map[string]string{"owned_by": "owned_by"},
		OutputOnlyAttrs:     map[string]bool{"allow_staff_override": true, "can_share": true, "can_update_basic": true, "can_update_restricted": true, "can_update_visibility": true, "can_view": true, "created": true, "created_by": true, "is_locked": true, "is_superadmin": true, "is_visible": true, "last_verified": true, "last_verified_by": true, "modified": true},
		SpreadAttrs:         map[string]bool{},
		CreateWritableAttrs: map[string]bool{"contacts": true, "definition": true, "description": true, "name": true, "owned_by": true, "type": true, "verified": true},
		UpdateWritableAttrs: map[string]bool{"definition": true, "description": true, "name": true, "owned_by": true, "type": true, "verified": true},
	}
}
