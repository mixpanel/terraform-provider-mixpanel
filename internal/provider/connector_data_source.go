...
import (
	...
	dsc "github.com/mixpanel/terraform-provider-mixpanel/internal/provider/datasource_connector"
)

var (
	_ datasource.DataSource              = (*ConnectorDataSource)(nil)
	_ datasource.DataSourceWithConfigure = (*ConnectorDataSource)(nil)
)

// keep the generated data-source schema package and schema builder imported.
var _ = dschema.StringAttribute{}

// NewConnectorDataSource constructs the connector data source.
...
func (d *ConnectorDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	s := dsc.ConnectorDataSourceSchema(ctx)
	s.Attributes["category_properties"] = dschema.StringAttribute{Optional: true, Computed: true}
	s.Attributes["connector_properties"] = dschema.StringAttribute{Optional: true, Computed: true}
	resp.Schema = s
}
...
func (d *ConnectorDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	id, err := stringAttrFromRaw(req.Config.Raw, "connector_id")
	if err != nil {
		resp.Diagnostics.AddError("Reading connector id", err.Error())
		return
	}
	path := strings.NewReplacer("{project_id}", projectID, "{connector_id}", id).Replace("/api/app/projects/{project_id}/connectors/{connector_id}")
	...
	if err != nil {
		resp.Diagnostics.AddError("Building connector state", err.Error())
		return
	}
	resp.State.Raw = val
	_ = tftypes.Value{}
