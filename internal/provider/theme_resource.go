...
import (
	...
	rsc "github.com/mixpanel/terraform-provider-mixpanel/internal/provider/resource_theme"
)
...
// NewThemeResource constructs the theme resource.
...
// projectID resolves the project from the theme project_id attribute (if any)
// falling back to the provider default.
func (r *ThemeResource) projectID(ctx context.Context, raw tftypes.Value) (string, error) {
...
func (r *ThemeResource) collectionPath(projectID string) string {
	return strings.NewReplacer("{project_id}", projectID).Replace("/api/app/projects/{project_id}/themes")
}

func (r *ThemeResource) instancePath(projectID, id string) string {
	return strings.NewReplacer("{project_id}", projectID, "{theme_id}", id).Replace("/api/app/projects/{project_id}/themes/{theme_id}")
}
...
func (r *ThemeResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError("Creating theme", err.Error())
		return
	}
	wire, err := unwrapTheme(respBody)
	if err != nil {
...
func (r *ThemeResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	id, err := stringAttrFromRaw(req.State.Raw, "theme_id")
	if err != nil {
...
func (r *ThemeResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError("Updating theme", err.Error())
		return
	}
	wire, err := unwrapTheme(respBody)
	if err != nil {
...
// idForTheme extracts the identity value from an unwrapped response body.
func idForTheme(wire map[string]any) string {
	if v, ok := client.IDFromWire(wire, "theme_id"); ok {
		return v
	}
...
// unwrapTheme unwraps the API envelope (when enveloped) and returns the body map.
func unwrapTheme(respBody []byte) (map[string]any, error) {
