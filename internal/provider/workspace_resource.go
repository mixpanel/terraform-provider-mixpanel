...
//
// HAND-EDITED EXCEPTION: the workspace PATCH schema REQUIRES `id` in the body
// (workspaces/util.py PATCH_WORKSPACE_SCHEMA: Required("id"), PREVENT_EXTRA).
// The bridge strips the identity attribute from wire bodies, so Update
// re-injects it after building the filtered body. Re-apply if regenerating.
...
import (
	...
	rsc "github.com/mixpanel/terraform-provider-mixpanel/internal/provider/resource_workspace"
)
...
// NewWorkspaceResource constructs the workspace resource.
...
func (r *WorkspaceResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	...
	normalizedJSON(s.Attributes, "data_filters")
...
// projectID resolves the project from the workspace project_id attribute (if any)
// falling back to the provider default.
func (r *WorkspaceResource) projectID(ctx context.Context, raw tftypes.Value) (string, error) {
...
func (r *WorkspaceResource) collectionPath(projectID string) string {
	return strings.NewReplacer("{project_id}", projectID).Replace("/api/app/projects/{project_id}/workspaces")
}

func (r *WorkspaceResource) instancePath(projectID, id string) string {
	return strings.NewReplacer("{project_id}", projectID, "{workspace_id}", id).Replace("/api/app/projects/{project_id}/workspaces/{workspace_id}")
}
...
func (r *WorkspaceResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError("Encoding workspace request", err.Error())
		return
	}
	respBody, err := r.client.Do(ctx, "POST", r.collectionPath(projectID), body)
	if err != nil {
		resp.Diagnostics.AddError("Creating workspace", err.Error())
		return
	}
	wire, err := unwrapWorkspace(respBody)
	if err != nil {
		resp.Diagnostics.AddError("Decoding workspace response", err.Error())
		return
	}
	id := idForWorkspace(wire)
	r.writeWorkspaceState(ctx, &resp.State, &resp.Diagnostics, client.MergeApply, req.Plan.Raw, wire, projectID, id)
}
...
func (r *WorkspaceResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	r.writeWorkspaceState(ctx, &resp.State, &resp.Diagnostics, client.MergeRead, req.State.Raw, wire, projectID, id)
}
...
func (r *WorkspaceResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// The workspace PATCH voluptuous schema marks `id` as Required in the BODY
	// (in addition to the path segment); the bridge strips identity attrs, so
	// re-add it here.
	body["id"] = jsonNumberOrString(id)
	respBody, err := r.client.Do(ctx, "PATCH", r.instancePath(projectID, id), body)
	if err != nil {
		resp.Diagnostics.AddError("Updating workspace", err.Error())
		return
	}
	wire, err := unwrapWorkspace(respBody)
	if err != nil {
...
func (r *WorkspaceResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if _, err := r.client.Do(ctx, "DELETE", r.instancePath(projectID, id), nil); err != nil {
		...
		resp.Diagnostics.AddError("Deleting workspace", err.Error())
		return
	}
}
...
// writeWorkspaceState turns an unwrapped API body into resource state. On
...
func (r *WorkspaceResource) writeWorkspaceState(ctx context.Context, state *tfsdk.State, diags *diagAppender, mode client.MergeMode, base tftypes.Value, wire map[string]any, projectID, id string) {
	...
	val, err := client.RawFromWireMerged(schemaType, mode, base, wire, extras, WorkspaceAttrSpec())
	if err != nil {
		diags.AddError("Building workspace state", err.Error())
		return
	}
	state.Raw = val
}

// idForWorkspace extracts the identity value from an unwrapped response body.
func idForWorkspace(wire map[string]any) string {
...
// unwrapWorkspace unwraps the API envelope (when enveloped) and returns the body map.
func unwrapWorkspace(respBody []byte) (map[string]any, error) {
