...
//
// HAND-EDITED EXCEPTION: the update verb is PATCH, not PUT. Live-verified
// 2026-07-02: PUT returns 405 "PUT is not currently implemented, use PATCH
// instead". Matches gen/refined_manifest.json (update: patch). Re-apply if
// regenerating from a stale manifest.
...
func (r *EmailDigestResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	...
	normalizedJSON(s.Attributes, "slack_subscriptions")
...
func (r *EmailDigestResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	r.writeEmailDigestState(ctx, &resp.State, &resp.Diagnostics, client.MergeApply, req.Plan.Raw, wire, projectID, id)
}
...
func (r *EmailDigestResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	r.writeEmailDigestState(ctx, &resp.State, &resp.Diagnostics, client.MergeRead, req.State.Raw, wire, projectID, id)
}
...
// writeEmailDigestState turns an unwrapped API body into resource state. On
...
func (r *EmailDigestResource) writeEmailDigestState(ctx context.Context, state *tfsdk.State, diags *diagAppender, mode client.MergeMode, base tftypes.Value, wire map[string]any, projectID, id string) {
	...
	val, err := client.RawFromWireMerged(schemaType, mode, base, wire, extras, EmailDigestAttrSpec())
	if err != nil {
