...
description: |-
  Manages a saved cohort defining a user segment based on behaviors and properties.
---
...
- every filter group must carry an **`event` clause** with `resourceType` and `value`
  (e.g. `event = {resourceType = "cohort", value = "$all_users", label = "All Users"}`) —
  a group missing the `event` clause is rejected with HTTP 400 "Missing or invalid 'resourceType'";
