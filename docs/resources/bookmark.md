...
description: |-
  Manages a saved report (bookmark) that can optionally appear on a dashboard.
---

# mixpanel_bookmark (Resource)

Manages a Mixpanel **saved report (bookmark)** (`mixpanel_bookmark`) — a report definition (insights, funnels, retention, flows, or other types) that can live standalone or as a cell on a board. See [mixpanel_dashboard](dashboard.md) for boards.
...
(the content API rejects invalid params with `400 InvalidParams`).
...
- multi-metric params (`params.sections.show[]`) apply the behavior and
  measurement rules from the [mixpanel_behavior](behavior.md) / [mixpanel_metric](metric.md) docs
