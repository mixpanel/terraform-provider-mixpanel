...
description: |-
  Manages a saved behavior definition used to define user segments and funnel steps.
---

# mixpanel_behavior (Resource)

Manages a Mixpanel **behavior** (`mixpanel_behavior`) — a saved behavioral definition (segment, funnel, or retention cohort) that can be referenced in reports and metrics.
...
Behaviors can be imported using the composite `PROJECT_ID:ID` format:

```terraform
import {
  to = mixpanel_behavior.power_users
  id = "1234567:890"
}
```

```bash
terraform import mixpanel_behavior.power_users 1234567:890
```
